"""From: https://www.kaggle.com/code/limitz/pytorch-dataset-with-volumetric-augmentations"""
...
def load_volume(dataset, labeled=True, slice_range=None):
    ...
    '''
    if labeled:
        path = os.path.join(dataset, "labels", "*", MASK_FILE_EXT)
    else:
        path = os.path.join(dataset, "images", "*", IMG_FILE_EXT)
...
dataset = RandomVolumetricDataset(TRAIN_DATASETS, shape=(TILE_SIZE, TILE_SIZE, TILE_SIZE), length=1000, transform=transform)

VOL_TRAIN_LOADER = torch.utils.data.DataLoader(dataset, batch_size=BATCH_SIZE, shuffle=True, num_workers=NUM_WORKERS, pin_memory=PIN_MEMORY)